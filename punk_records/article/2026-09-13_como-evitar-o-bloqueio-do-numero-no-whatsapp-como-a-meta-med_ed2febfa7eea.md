---
item_id: "26233c12-9f86-4946-ae5a-9f3308169a86"
platform: article
external_id: "ed2febfa7eea"
...
# Como evitar o bloqueio do número no WhatsApp: como a Meta mede reputação, 7 causas e 8 práticas (Dito)

🔗 https://blog.dito.com.br/como-evitar-bloqueio-no-whatsapp

## Resumo

Artigo da Dito, empresa de CRM para varejo, escrito por Pedro Ivo Martins, sobre por que a Meta bloqueia números de WhatsApp usados comercialmente e como evitar. A tese central é que a Meta não julga uma mensagem isolada e sim o comportamento acumulado do número, como o sistema de pontos da carteira de motorista: ela mede em tempo real quantos destinatários bloqueiam, quantos denunciam como spam, quantos respondem, além do volume e da qualidade do conteúdo, e disso sai uma nota de reputação. Bloqueio temporário funciona como advertência reversível; disparo em massa sem autorização, ferramenta não oficial e reincidência levam ao banimento permanente, com perda definitiva do número. O texto lista sete causas de bloqueio — disparo sem opt-in, APIs não homologadas, mensagens repetitivas e robóticas, pico de envio em número novo sem aquecimento, ausência de descadastro, categorias restritas e reutilização de número já penalizado — e oito práticas de prevenção, das quais as mais concretas são capturar o opt-in pedindo ao cliente que salve o contato ainda no atendimento, priorizar quem tem propensão de compra e personalizar a mensagem. Distingue o aplicativo WhatsApp Business (atendimento humano, um número, sem escala) da API oficial (templates aprovados, volume, múltiplos atendentes) e alerta que a API não imuniza contra bloqueio, porque template irrelevante para lista sem opt-in derruba a reputação do mesmo jeito. A segunda metade do artigo é conteúdo comercial da própria Dito: matriz de volume × receita influenciada, RFM, IA de propensão e CDP. Traz um benchmark de conversão do WhatsApp em CRM — 0,59% baixa performance, 2,45% média, 6,16% alta — sem descrever a metodologia nem o tamanho da amostra.

## Tópicos

- **Reputação por acúmulo, não por mensagem** — A Meta monitora em tempo real bloqueios, denúncias, respostas, volume e qualidade, e forma uma nota de reputação do número. Uma mensagem não derruba a conta; o padrão repetido derruba.
- **Temporário x permanente** — O bloqueio temporário é advertência reversível se as práticas mudarem. Disparo em massa sem autorização, ferramenta não homologada e reincidência podem virar banimento permanente, e o número se perde.
- **As regras exatas não são públicas** — A Meta não divulga o critério do algoritmo, pela mesma lógica de um antifraude bancário. O que é público são os comportamentos de risco.
- **Sete causas de bloqueio** — Disparo sem opt-in, APIs e automações não oficiais, mensagens repetitivas e robóticas, pico de envio em número novo, falta de descadastro claro, produtos e serviços restritos, e reutilização de número já penalizado.
- **Opt-in pedindo para salvar o contato** — A prática mais concreta do artigo: ainda no atendimento, o vendedor manda um oi e pede ao cliente que salve o número. Mensagem de contato salvo tem menos chance de virar denúncia.
- **Aplicativo x API oficial** — O app serve ao atendimento humano e contextual; a API serve a volume com templates aprovados. A API não imuniza: template irrelevante para lista sem opt-in derruba a reputação igual.
- **Conteúdo comercial da Dito** — Matriz volume × receita influenciada, RFM, IA de propensão, CDP e benchmark de conversão (0,59% / 2,45% / 6,16%) sem metodologia declarada — a parte do artigo que vende a plataforma.

## Ferramentas citadas

- **WhatsApp Business (aplicativo)**: atendimento humano em um número, sem escala; é o que um prestador solo usa
- **WhatsApp Business Platform (API oficial)**: envio em volume por templates aprovados pela Meta, contratada via provedor homologado
- **Dito CDP**: produto do autor: unifica dados on e offline e prioriza contatos por RFM e IA

## Pontos-chave

- A nota de reputação do número vem de como os destinatários reagem: bloqueios recebidos, denúncias de spam e taxa de resposta pesam mais que o texto da mensagem.
- Número novo que dispara volume logo após a ativação aciona alerta automático, porque é o padrão de um spammer; números novos precisam de aquecimento gradual.
- Mesma mensagem em escala, sempre no mesmo horário e sem variação é lida como automação abusiva — personalizar reduz o risco.
- Disparo para quem não autorizou o contato é apontado como a causa número um de bloqueio.
- Ferramenta ou API não homologada pela Meta viola os termos e costuma gerar suspensão imediata.
- Número que já foi penalizado carrega histórico negativo: reaproveitar a linha começa a operação em desvantagem.
- Bloqueio temporário é reversível corrigindo a prática; a velocidade da reação ao primeiro alerta é o que separa advertência de banimento.
- A API oficial protege contra riscos de escala (volume, ritmo, aprovação de conteúdo), não contra má prática.
- O benchmark de conversão citado (0,59% / 2,45% / 6,16%) vem sem metodologia nem amostra e é da própria fornecedora.
- A segunda metade do artigo é venda da plataforma da Dito e não deve ser lida como recomendação neutra.

## Como aplicar

O artigo é escrito para varejo com base própria, mas quatro regras atingem em cheio a prospecção fria de sites em Vitória da Conquista: número novo não dispara volume, mensagem idêntica repetida em sequência é padrão robótico, quem recebe sem ter salvado o contato é quem mais denuncia, e número já penalizado nunca volta ao zero. Na abordagem do Jardins Café isso significa uma conversa por vez pelo número pessoal já aquecido, texto reescrito a cada alvo, e nada de lista. Se um dia a prospecção virar volume, a decisão passa a ser API oficial com opt-in, não app com disparo.

## 🪖 Shaka diz

O que vale aqui é a mecânica: a Meta pontua o número pela reação de quem recebe, não pelo que você escreve. Três regras se aplicam a você hoje — não usar número novo, não repetir a mesma mensagem em sequência e não montar lista. A segunda metade do artigo é venda de CDP e o benchmark de conversão vem sem metodologia; ignore os números, guarde as causas.
