---
item_id: "32ff23f9-381f-499d-9b24-6b582b47ce3a"
platform: article
external_id: "095ffd77aa1a"
canonical_url: "https://serpapi.com/search-api"
channel: "SerpApi"
captured_at: 2026-08-27
status: enriched
triage: null
tags: ["serpapi", "api-reference", "google-search", "parametros", "json", "documentacao"]
applicability:
  saas_pessoal: alta
...
# SerpApi — referência da Google Search Engine Results API: parâmetros, tipos de busca e estrutura do JSON

🔗 https://serpapi.com/search-api

## Resumo

Documentação completa do endpoint https://serpapi.com/search?engine=google. Parâmetros: q (obrigatório, aceita operadores como site:, inurl:, intitle: e parâmetros avançados as_*); localização por location (cidade, recomendado), uule (código do Google), lat/lon (exigem gl) e radius (raio em metros, limites por dispositivo); parâmetros avançados ludocid (CID de um lugar), lsig, kgmid (ID do Knowledge Graph), si e ibp; localização de idioma e país com google_domain, gl (país), hl (idioma), cr e lr (restringir países/idiomas); filtros tbs (datas, patentes, notícias, imagens…), safe e nfpr; tipo de busca tbm (isch imagens, lcl local, vid vídeos, nws notícias, shop shopping, pts patentes); parâmetros da SerpApi como device (desktop/tablet/mobile) e no_cache (cache é servido só quando todos os parâmetros coincidem). O restante da página documenta paginação e a estrutura do JSON de resposta (organic_results, knowledge_graph, local_results, related_questions etc.) com exemplos. Texto integral guardado (88k caracteres, cortado no limite do vault).

## Tópicos

- **Consulta** — q obrigatório; operadores site:, inurl:, intitle:; parâmetros avançados as_*.
- **Localização** — location (cidade), uule, lat/lon + radius; gl para consistência de país.
- **País, idioma e filtros** — google_domain, gl, hl, cr, lr; tbs, safe, nfpr.
- **Tipo de busca e opções** — tbm: isch/lcl/vid/nws/shop/pts; device; no_cache; ludocid/kgmid para lugares e Knowledge Graph.
- **Resposta** — JSON com organic_results, knowledge_graph, local_results, related_questions e outros blocos, com exemplos.

## Ferramentas citadas

- **SerpApi playground**: demo interativa do endpoint
- **UULE Generator**: converter local/coordenadas em uule

## Pontos-chave

- Uma chamada = uma página do Google como JSON; tbm troca o 'motor' (imagens, local, notícias, shopping) sem mudar o endpoint.
- Para resultados consistentes por país, combinar location com gl.
- no_cache força busca nova (e cobra); sem ele, buscas idênticas são grátis.
- ludocid/kgmid permitem puxar diretamente a ficha de um lugar ou entidade.

## Como aplicar

Guardado como referência para quando Atlas for construir: os parâmetros location+gl+hl+tbm=lcl cobrem o caso de prospecção por cidade; ler a seção de resposta antes de definir o schema no SaaS.

## 💡 Edison diz

Isso é manual, não faísca — mas manual guardado é faísca futura. Quando a Atlas soldar a busca por cidade, os parafusos estão aqui: location + gl + tbm=lcl. Não li as 88 mil letras inteiras, chefe; o Punk Records leu por mim, e é para isso que ele existe.
...
Parameter can't be used together with uule, lat, and lon parameters.
...
For consistent results with coordinates-based uule parameter, a matching gl parameter is required.

Head to UULE Generator if you need to convert a location or GPS coordinates to uule parameter, and vice versa.

lat

Optional

Parameter defines a GPS latitude for the search origin.

Parameter is required when using lon parameter.

For consistent results, a matching gl parameter is required.

Parameter can't be used together with location and uule parameters.

lon

Optional

Parameter defines a GPS longitude for the search origin.
...
Parameter defines the distance (in meters) within which to bias the search results. Results inside of this region will be ranked higher than results outside of it. However, prominent results from outside of the search radius may still be included.

Value range per device:
...
Parameter can be used alongside location or lat/lon parameters.
...
It can be set to:`(no tbm parameter)`: regular Google Search,`isch`: Google Images API,`lcl`: Google Local API,`vid`: Google Videos API,`nws`: Google News API,`shop`: Google Shopping API,`pts`: Google Patents API,
