---
item_id: "f49063c7-d706-4d12-8469-032f26e9d4cd"
platform: article
external_id: "8449d3c9e81a"
...
# Memória do Claude Code: CLAUDE.md, regras por caminho e auto memory

🔗 https://docs.claude.com/en/docs/claude-code/memory

## Resumo

A documentação oficial explica os dois sistemas de memória do Claude Code, carregados no início de toda sessão: os arquivos CLAUDE.md (instruções que você escreve) e a auto memory (notas que o próprio Claude escreve a partir das suas correções). Ambos são contexto, não configuração imposta — para bloquear uma ação de verdade é preciso um hook PreToolUse. CLAUDE.md pode existir em vários escopos (organização, usuário em ~/.claude, projeto em ./CLAUDE.md ou ./.claude/CLAUDE.md, pessoal em CLAUDE.local.md), concatenados da raiz até a pasta atual; arquivos em subpastas só entram quando o Claude lê arquivos dali. A recomendação é manter cada arquivo abaixo de 200 linhas, com instruções específicas e verificáveis ("rode npm test antes do commit", não "teste suas mudanças"), sem regras contraditórias, e mover detalhes para `.claude/rules/` com frontmatter `paths` para carregar só quando arquivos correspondentes forem tocados. Importações com `@caminho` expandem outros arquivos (até 4 níveis) e AGENTS.md pode ser importado assim. A auto memory guarda quatro tipos de nota (user, feedback, project, reference) em ~/.claude/projects/<projeto>/memory/, com um MEMORY.md índice limitado a 200 linhas/25 KB no carregamento; os arquivos-tópico são lidos sob demanda. Para diagnosticar, `/context` mostra o que carregou, `/memory` abre e edita os arquivos, e `/doctor` propõe cortes no CLAUDE.md.

## Tópicos

- **CLAUDE.md vs auto memory** — Dois sistemas complementares, ambos carregados a cada sessão como contexto (não enforcement); hooks bloqueiam de verdade.
- **Escopos e ordem de carga** — Organização (managed) → usuário (~/.claude) → projeto → CLAUDE.local.md; concatenados da raiz até a pasta atual; subpastas carregam sob demanda.
- **Como escrever instruções** — Menos de 200 linhas por arquivo, cabeçalhos e bullets, instruções verificáveis, sem contradições; claudeMdExcludes para monorepos.
- **Importações e AGENTS.md** — Sintaxe @caminho (até 4 níveis; ignorada dentro de backticks); CLAUDE.md que importa AGENTS.md evita duplicar instruções entre agentes.
- **Regras em .claude/rules/** — Um arquivo por tema; frontmatter `paths` com globs carrega a regra só quando arquivos correspondentes são lidos; symlinks e ~/.claude/rules/ para regras pessoais.
- **Auto memory** — Tipos user/feedback/project/reference em ~/.claude/projects/<projeto>/memory/; MEMORY.md como índice (200 linhas/25 KB); tópicos lidos sob demanda; toggle em /memory.
- **Diagnóstico** — /context lista os arquivos de memória carregados; /memory edita; /doctor sugere cortes; CLAUDE.md da raiz sobrevive ao /compact.

## Ferramentas citadas

- **CLAUDE.md**: arquivo de instruções persistentes por escopo (org, usuário, projeto, local)
- **.claude/rules/**: regras modulares, opcionalmente limitadas a caminhos via frontmatter paths
- **Auto memory (MEMORY.md)**: notas que o Claude escreve e relê entre sessões
- **/context**: confere quais arquivos de memória entraram na sessão
- **/memory**: lista, abre e edita CLAUDE.md e auto memory; liga/desliga auto memory
- **/doctor**: propõe cortes no CLAUDE.md (remove o que o Claude deduz do código)
- **PreToolUse hook**: único mecanismo que bloqueia uma ação de fato, ao contrário do CLAUDE.md

## Pontos-chave

- CLAUDE.md e auto memory são contexto, não configuração: para proibir algo de verdade, use um hook PreToolUse.
- Ordem de carga: managed → ~/.claude/CLAUDE.md → CLAUDE.md dos diretórios acima → projeto → CLAUDE.local.md; o mais próximo da pasta atual é lido por último.
- Alvo de menos de 200 linhas por CLAUDE.md; arquivos maiores gastam contexto e reduzem a obediência; acima de 4 MiB o arquivo é ignorado.
- Instruções concretas e verificáveis funcionam melhor que genéricas; regras contraditórias fazem o Claude escolher uma ao acaso.
- `.claude/rules/*.md` com frontmatter `paths` carrega a regra só quando arquivos que batem no glob são lidos — economiza contexto.
- Importar arquivos com `@caminho` organiza, mas não economiza contexto: o importado carrega no início do mesmo jeito.
- Comentários HTML em bloco no CLAUDE.md são removidos antes de ir ao contexto — servem para notas de manutenção sem custo.
- Auto memory: MEMORY.md carrega só as primeiras 200 linhas/25 KB; os arquivos-tópico são lidos sob demanda; é local à máquina.
- Quando você diz "lembre que…", vai para a auto memory; "adicione ao CLAUDE.md" vai para o arquivo de instruções.
- Após /compact, o CLAUDE.md da raiz é relido do disco; instruções dadas só na conversa se perdem — leve-as para o CLAUDE.md.

## Como aplicar

No Vegapunk e nos projetos do SaaS/cliente: manter o CLAUDE.md abaixo de 200 linhas (o global já recebeu o bloco do Punk Records — conferir o tamanho), mover regras específicas por tipo de arquivo para .claude/rules/ com `paths`, e trocar qualquer regra 'de verdade' (ex.: nunca fazer push sem 'push') por um hook PreToolUse em vez de confiar só no texto.

## 📚 Pythagoras diz

Registrado, Fernando: este é o manual do próprio mecanismo que faz os Satélites existirem — e ele diz, sem rodeios, que instrução escrita é contexto, não lei. O registro aponta duas coisas que deduzo importarem para nós: o CLAUDE.md global já carrega o bloco do Punk Records e precisa ficar abaixo de 200 linhas, e a regra 'só push com a palavra push' seria mais segura como hook do que como frase. Marco a lacuna: ainda não há registro de hooks no Punk Records.
...
`/context` and confirm `CLAUDE.md` appears under **Memory files**. On Windows, creating a symlink requires Administrator privileges or Developer Mode, so use the
...
`CLAUDE.md`, `.claude/CLAUDE.md`, `.claude/rules/*.md`, and `CLAUDE.local.md` from the additional directory. `CLAUDE.local.md` is skipped if you exclude `local` from `--setting-sources`.
...
`paths` frontmatter are loaded at launch with the same priority as `.claude/CLAUDE.md`.
...
`src/*.{ts,tsx}` expands to two patterns, and `{a,b}/{c,d}/*.{ts,tsx}` to eight. To keep expansion bounded, a rule’s whole `paths` list shares one budget of 1,000 expanded patterns and 4 MiB, and patterns without braces don’t count against it.
...
`claudeMd` key lets you put managed CLAUDE.md content directly inside `managed-settings.json` instead of deploying a separate file.
...
`claudeMdExcludes` at any settings layer: user, project, local, or managed policy. Arrays merge across layers.
...
`~/`. When you set it in a project’s `.claude/settings.json` or `.claude/settings.local.json`, Claude Code honors it under the same workspace trust rule as hooks in settings files.
...
Claude Code deletes old session transcripts after the `cleanupPeriodDays` retention period, but excludes the files in the memory directory from that retention sweep. `MEMORY.md` and topic files stay until you or Claude edits or deletes them.
#### How it works
